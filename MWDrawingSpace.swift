    ...
    override func mouseMoved(with theEvent: NSEvent) {
       ...
       //set up the correct Cursor
        ...
        if drawTempArcStartPtSet == true && drawTempArcEndPtSet == false {
            ...
            let dX = tempArc.modelArc.centerPt.x - tempArc.modelArc.endPt.x
            let dY = tempArc.modelArc.centerPt.y - tempArc.modelArc.endPt.y
            ...
            Swift.print("Calculated R = \(mouseBasedRadModel)")
    ...
    //vars for 3 finger drag i.e. pan
        ...
        override func touchesMoved(with event: NSEvent) {
            ...
            if touchArray.count == 3{// code for a 3 finger drag, i.e. pan
                ...
                if fireCount == 0{
                    let touchStart0 = (touchArray[0] as AnyObject).normalizedPosition
                    let touchStart1 = (touchArray[1] as AnyObject).normalizedPosition
                    let touchStart2 = (touchArray[2] as AnyObject).normalizedPosition
                    
                    touchStart = touchStart0!
                    
                
                    if Float((touchStart1?.x)!) < Float((touchStart0?.x)!){
                        touchStart = touchStart1!
                        if (touchStart2?.x)! < touchStart.x{
                            touchStart = touchStart2!
                        }
                    }else if Float((touchStart2?.x)!) < Float((touchStart0?.x)!){
                        touchStart = touchStart2!
                    }
                    ...
                    let touchMid0 = (touchArray[0] as AnyObject).normalizedPosition
                    let touchMid1 = (touchArray[1] as AnyObject).normalizedPosition
                    let touchMid2 = (touchArray[2] as AnyObject).normalizedPosition
                    
                    touchMid = touchMid0!
                    
                    
                    if Float((touchMid1?.x)!) < Float((touchMid0?.x)!){
                        touchMid = touchMid1!
                        if (touchMid2?.x)! < touchMid.x{
                            touchMid = touchMid2!
                        }
                    }else if Float((touchMid2?.x)!) < Float((touchMid0?.x)!){
                        touchMid = touchMid2!
                    }
                    ...
                    if abs(runningDelta.x) > 0.001 || abs(runningDelta.y) > 0.001{
                        updateEntityTransforms(masterEntityTable)
                        updateEntityTransforms(unselectedEntityTable)
                        display()
